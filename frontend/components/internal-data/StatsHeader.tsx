        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          <div className="flex items-center gap-2 overflow-x-auto py-1">
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-slate-50 border border-slate-200 text-xs">
              ...
              <span className="text-slate-500">{t('statsHeader.tables')}</span>
              <span className="font-bold text-slate-900">{stats.totalCollections}</span>
            </div>

            <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-slate-50 border border-slate-200 text-xs">
              <Server className="w-3.5 h-3.5 text-sky-600" />
              <span className="text-slate-500">{t('statsHeader.recordsInDb')}</span>
              <span className="font-bold text-slate-900">{stats.totalRecords.toLocaleString()}</span>
            </div>

            <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-slate-50 border border-slate-200 text-xs">
              <Zap className="w-3.5 h-3.5 text-amber-600" />
              <span className="text-slate-500">{t('statsHeader.apiEndpoints')}</span>
              <span className="font-bold text-slate-900">{stats.totalEndpoints}</span>
            </div>

            <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-slate-50 border border-slate-200 text-xs">
              <RefreshCw className="w-3.5 h-3.5 text-emerald-600" />
              <span className="text-slate-500">{t('statsHeader.autoSync')}</span>
              <span className="font-bold text-slate-900">{t('statsHeader.active', { count: stats.activeSchedules })}</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2.5">
            <button
              ...
              title={t('statsHeader.refreshTitle')}
            ...
            >
              ...
              <span>{t('statsHeader.connectNewTable')}</span>
            </button>
          </div>
