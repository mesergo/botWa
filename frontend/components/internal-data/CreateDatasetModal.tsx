...
export const CreateDatasetModal: React.FC<CreateDatasetModalProps> = ({ token, isOpen, onClose, onCreated }) => {
  ...
  const handlePreviewSheet = async () => {
    if (!googleSheetsUrl.trim()) { setPreviewError(t('createDataset.needGoogleSheetsUrl')); return; }
    ...
    try {
      ...
      setPreviewData(res);
      if (!name) setName('Google Sheet - ' + new Date().toLocaleDateString(getFormatLocale()));
      ...
      setPreviewError(err.message || t('createDataset.sheetLoadError'));
    } finally {
  ...
  const loadPresetTemplate = (templateType: string) => {
    if (templateType === 'crm') {
      setName(t('createDataset.sampleCrmName'));
      setDescription(t('createDataset.sampleCrmDescription'));
    } else if (templateType === 'products') {
      setName(t('createDataset.sampleInventoryName'));
      setDescription(t('createDataset.sampleInventoryDescription'));
    }
  };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim()) { setSubmitError(t('createDataset.needTableName')); return; }
    ...
    try {
      ...
      if (activeTab === 'google_sheets') {
        if (!googleSheetsUrl.trim()) throw new Error(t('createDataset.needGoogleSheetsUrl2'));
        ...
        if (!uploadedFile) throw new Error(t('createDataset.needUploadFile'));
        ...
        if (!res.ok) throw new Error(data.error || t('createDataset.importFileError'));
      } else {
        createdTable = await createTable(token, {
          name: name.trim() || t('createDataset.defaultCrmTableName'),
          description: description.trim() || t('createDataset.defaultCrmTableDescription'),
          source_type: 'manual',
          api: { enabled: isPublicApi },
        });
        const sampleRows = [
      ...
      setSubmitError(err.message || t('createDataset.createTableError'));
    } finally {
      ...
      <div className="relative w-full max-w-2xl bg-white border border-slate-200 rounded-3xl shadow-xl overflow-hidden my-8">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-slate-50/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-slate-900">{t('createDataset.title')}</h2>
              <p className="text-xs text-slate-500">{t('createDataset.subtitle')}</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-3 gap-1 p-1.5 m-6 mb-4 bg-slate-100 rounded-2xl border border-slate-200">
          ...
          <button type="button" onClick={() => setActiveTab('file_upload')} className={`flex items-center justify-center gap-2 py-2 px-3 rounded-xl text-xs font-semibold transition ${activeTab === 'file_upload' ? 'bg-white text-indigo-700 shadow-2xs font-bold' : 'text-slate-600 hover:text-slate-900'}`}>
            <FileSpreadsheet className="w-4 h-4 text-sky-600" /><span>{t('createDataset.uploadFileTab')}</span>
          </button>
          <button type="button" onClick={() => setActiveTab('sample_templates')} className={`flex items-center justify-center gap-2 py-2 px-3 rounded-xl text-xs font-semibold transition ${activeTab === 'sample_templates' ? 'bg-white text-indigo-700 shadow-2xs font-bold' : 'text-slate-600 hover:text-slate-900'}`}>
            <Sparkles className="w-4 h-4 text-amber-500" /><span>{t('createDataset.sampleTemplateTab')}</span>
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="px-6 pb-6 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-medium text-slate-700 mb-1.5">{t('createDataset.tableNameLabel')}</label>
              <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder={t('createDataset.tableNamePlaceholder')} className="w-full px-3.5 py-2 text-sm bg-white border border-slate-300 rounded-xl text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500" required />
            </div>
            <div>
              <label className="block text-xs font-medium text-slate-700 mb-1.5">{t('createDataset.descriptionLabel')}</label>
              <input type="text" value={description} onChange={(e) => setDescription(e.target.value)} placeholder={t('createDataset.descriptionPlaceholder')} className="w-full px-3.5 py-2 text-sm bg-white border border-slate-300 rounded-xl text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
            </div>
          </div>
            ...
            <div className="space-y-4 p-4 bg-slate-50 rounded-2xl border border-slate-200">
              <div>
                <label className="block text-xs font-medium text-slate-700 mb-1.5 flex items-center justify-between">
                  <span className="flex items-center gap-1.5"><Link className="w-3.5 h-3.5 text-indigo-600" />{t('createDataset.googleSheetsLinkLabel')}</span>
                  <span className="text-[11px] text-slate-500">{t('createDataset.googleSheetsPublicHint')}</span>
                </label>
                <div className="flex gap-2">
                  ...
                  <button type="button" onClick={handlePreviewSheet} disabled={isPreviewing || !googleSheetsUrl} className="px-3.5 py-2 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold rounded-xl flex items-center gap-1.5 transition disabled:opacity-50 shadow-xs">
                    ...
                    <span>{t('createDataset.checkLink')}</span>
                  </button>
                </div>
              </div>
              ...
              {previewError && (
                <div className="p-3 bg-rose-50 border border-rose-200 rounded-xl text-xs text-rose-700 flex items-start gap-2">
                  ...
                  <div><span className="font-semibold">{t('createDataset.sheetAccessError')}</span>{previewError}</div>
                </div>
              )}
              ...
              {previewData && (
                <div className="p-3 bg-emerald-50 border border-emerald-200 rounded-xl space-y-2">
                  <div className="flex items-center justify-between text-xs text-emerald-800 font-semibold">
                    <span className="flex items-center gap-1.5"><CheckCircle2 className="w-4 h-4 text-emerald-600" />{t('createDataset.sheetReadSuccess', { rows: previewData.totalRows, cols: previewData.headers.length })}</span>
                  </div>
              ...
              <div className="pt-2 border-t border-slate-200 space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <h4 className="text-xs font-semibold text-slate-900">{t('createDataset.autoSyncScheduleTitle')}</h4>
                      <p className="text-[11px] text-slate-500">{t('createDataset.autoSyncScheduleHint')}</p>
                    </div>
                  </div>
                ...
                {autoSyncEnabled && (
                  <div className="grid grid-cols-2 gap-3 p-3 bg-white rounded-xl border border-slate-200">
                    <div>
                      <label className="block text-[11px] font-medium text-slate-700 mb-1">{t('createDataset.updateFrequencyLabel')}</label>
                      <select value={intervalMinutes} onChange={(e) => setIntervalMinutes(Number(e.target.value))} className="w-full px-3 py-1.5 text-xs bg-white border border-slate-300 rounded-lg text-slate-800">
                        <option value={1}>{t('createDataset.every1Minute')}</option><option value={5}>{t('createDataset.every5Minutes')}</option><option value={15}>{t('createDataset.every15Minutes')}</option>
                        <option value={30}>{t('createDataset.every30Minutes')}</option><option value={60}>{t('createDataset.every1Hour')}</option><option value={360}>{t('createDataset.every6Hours')}</option><option value={1440}>{t('createDataset.every24Hours')}</option>
                      </select>
                    </div>
                    <div>
                      <label className="block text-[11px] font-medium text-slate-700 mb-1">{t('createDataset.syncMethodLabel')}</label>
                      <select value={syncMode} onChange={(e) => setSyncMode(e.target.value as InternalDataSyncMode)} className="w-full px-3 py-1.5 text-xs bg-white border border-slate-300 rounded-lg text-slate-800">
                        <option value="replace">{t('createDataset.syncReplaceAll')}</option>
                        <option value="upsert">{t('createDataset.syncUpsert')}</option>
                        <option value="append">{t('createDataset.syncAppendOnly')}</option>
                      </select>
                    </div>
                    {syncMode === 'upsert' && (
                      <div className="col-span-2">
                        <label className="block text-[11px] font-medium text-slate-700 mb-1">{t('createDataset.uniqueKeyFieldLabel')}</label>
            ...
            <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200 space-y-4">
              <div className="border-2 border-dashed border-slate-300 hover:border-indigo-500 rounded-2xl p-6 text-center transition cursor-pointer relative bg-white">
                ...
                <div className="flex flex-col items-center">
                  ...
                  <p className="text-xs font-semibold text-slate-900">{t('createDataset.dragFileHint')}</p>
                  <p className="text-[11px] text-slate-500 mt-1">{t('createDataset.supportedFormatsHint')}</p>
                </div>
              </div>
              {uploadedFile && (
          ...
          {activeTab === 'sample_templates' && (
            <div className="grid grid-cols-2 gap-3 p-4 bg-slate-50 rounded-2xl border border-slate-200">
              <div onClick={() => loadPresetTemplate('crm')} className="p-3.5 rounded-xl bg-white hover:bg-slate-50 border border-slate-200 hover:border-indigo-400 cursor-pointer transition shadow-2xs">
                <div className="flex items-center gap-2 mb-1.5"><span className="p-1.5 rounded-lg bg-indigo-50 text-indigo-600">👥</span><h4 className="text-xs font-bold text-slate-900">{t('createDataset.crmSampleTitle')}</h4></div>
                <p className="text-[11px] text-slate-500">{t('createDataset.crmSampleHint')}</p>
              </div>
              <div onClick={() => loadPresetTemplate('products')} className="p-3.5 rounded-xl bg-white hover:bg-slate-50 border border-slate-200 hover:border-indigo-400 cursor-pointer transition shadow-2xs">
                <div className="flex items-center gap-2 mb-1.5"><span className="p-1.5 rounded-lg bg-emerald-50 text-emerald-600">📦</span><h4 className="text-xs font-bold text-slate-900">{t('createDataset.inventorySampleTitle')}</h4></div>
                <p className="text-[11px] text-slate-500">{t('createDataset.inventorySampleHint')}</p>
              </div>
            </div>
          )}
          ...
          <div className="flex items-center justify-between p-3 bg-slate-50 rounded-2xl border border-slate-200">
            <div>
              <span className="text-xs font-medium text-slate-900">{t('createDataset.apiAccessLabel')}</span>
              <p className="text-[11px] text-slate-500">{t('createDataset.apiAccessHint')}</p>
            </div>
            <label className="relative inline-flex items-center cursor-pointer">
          ...
          <div className="flex items-center justify-end gap-3 pt-3 border-t border-slate-100">
            <button type="button" onClick={onClose} className="px-4 py-2 text-xs font-medium text-slate-600 hover:text-slate-900 hover:bg-slate-100 rounded-xl transition">{t('createDataset.cancel')}</button>
            <button type="submit" disabled={isSubmitting} className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold rounded-xl shadow-xs transition flex items-center gap-2 disabled:opacity-50">
              {isSubmitting ? (<><Loader2 className="w-4 h-4 animate-spin" /><span>{t('createDataset.creating')}</span></>) : (<><CheckCircle2 className="w-4 h-4" /><span>{t('createDataset.createTable')}</span></>)}
            </button>
          </div>
        </form>
      </div>
