    ...
    try {
      ...
      if (isRawJsonMode) {
        try {
          ...
          throw new Error(t('recordModal.invalidJson'));
        }
      }
      ...
      setError(err.message || t('recordModal.saveError'));
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const renderInput = (field: InternalDataField) => {
    ...
    return (
      <input
        ...
        placeholder={t('recordModal.fieldPlaceholder', { field: field.label || field.key })}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-xs overflow-y-auto">
      <div className="relative w-full max-w-lg bg-white border border-slate-200 rounded-3xl shadow-xl overflow-hidden my-8">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-slate-50/50">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-sm font-bold text-slate-900">
                {isEditing ? t('recordModal.editTitle') : t('recordModal.addTitle')}
              </h3>
              <p className="text-[11px] text-slate-500">{t('recordModal.table', { name: table.name })}</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <span>{isRawJsonMode ? t('recordModal.formMode') : t('recordModal.jsonMode')}</span>
            </button>
        ...
        <form onSubmit={handleSave} className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
          ...
          {isRawJsonMode ? (
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1">{t('recordModal.jsonDataLabel')}</label>
              <textarea
                value={rawJsonText}
          ...
          <div className="flex items-center justify-end gap-2 pt-4 border-t border-slate-100">
            ...
            <button type="submit" disabled={isSaving} className="px-5 py-2 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold rounded-xl shadow-xs transition flex items-center gap-1.5 disabled:opacity-50">
              ...
              <span>{isEditing ? t('recordModal.saveChanges') : t('recordModal.addRecord')}</span>
            </button>
          </div>
        </form>
