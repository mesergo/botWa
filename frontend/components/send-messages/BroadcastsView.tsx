    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-black text-slate-900">{t('broadcasts.title')}</h1>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          >
            <option value="">{t('broadcasts.allLists')}</option>
            {groups.map(g => (
          ...
          >
            <RotateCcw size={15} className={allBroadcastsLoading ? 'animate-spin' : ''} /> {t('broadcasts.refresh')}
          </button>
        </div>
      </div>
      ...
      {allBroadcastsLoading ? (
        ...
        <div className="py-24 bg-white border-2 border-dashed border-slate-200 rounded-[2.5rem] flex flex-col items-center justify-center gap-4 text-slate-300">
          ...
          <p className="text-xl font-bold">{t('broadcasts.empty')}</p>
        </div>
      ) : (
        <div className="bg-white border border-slate-100 rounded-2xl overflow-hidden shadow-sm">
          <div className="grid grid-cols-[9rem_1fr_5rem_4rem_4rem_4rem_4rem_5rem_5rem] gap-2 px-6 py-3 bg-slate-50 border-b border-slate-100 text-xs font-bold text-slate-400 uppercase tracking-wide">
            <span>{t('broadcasts.columns.date')}</span>
            <span>{t('broadcasts.columns.content')}</span>
            <span>{t('broadcasts.columns.list')}</span>
            <span>{t('broadcasts.columns.total')}</span>
            <span>{t('broadcasts.columns.sent')}</span>
            <span>{t('broadcasts.columns.failed')}</span>
            <span>{t('broadcasts.columns.skipped')}</span>
            <span>{t('broadcasts.columns.schedule')}</span>
            <span>{t('broadcasts.columns.status')}</span>
          </div>
          {filtered.map((b, idx) => {
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2 text-xs font-bold text-slate-600">
                  ...
                  {new Date(b.createdAt).toLocaleString(getFormatLocale(i18n.resolvedLanguage), { dateStyle: 'short', timeStyle: 'short' })}
                </div>
                <div className="min-w-0">
                  {b.is_template ? (
                    <div className="flex items-start gap-2 flex-wrap">
                      <span className="px-2 py-0.5 bg-purple-100 text-purple-700 rounded-md text-xs font-black flex-shrink-0">{t('broadcasts.template')}</span>
                ...
                <span>
                  {b.scheduled_at ? (
                    <div className="flex flex-col gap-0.5">
                      <span className="px-2 py-0.5 bg-blue-100 text-blue-700 rounded-md text-xs font-black w-fit">{t('broadcasts.schedule.scheduled')}</span>
                      <span className="text-xs font-semibold text-blue-600 whitespace-nowrap">
                        {new Date(b.scheduled_at).toLocaleString(getFormatLocale(i18n.resolvedLanguage), { dateStyle: 'short', timeStyle: 'short' })}
                      </span>
                    </div>
                  ) : (
                    <span className="px-2 py-0.5 bg-slate-100 text-slate-500 rounded-md text-xs font-semibold">{t('broadcasts.schedule.immediate')}</span>
                  )}
                </span>
                <span>
                  {b.status === 'cancelled' && <span className="px-2 py-0.5 bg-red-100 text-red-600 rounded-md text-xs font-black">{t('broadcasts.status.cancelled')}</span>}
                  {b.status === 'completed' && <span className="px-2 py-0.5 bg-green-100 text-green-700 rounded-md text-xs font-black">{t('broadcasts.status.completed')}</span>}
                  {isStopped && <span className="px-2 py-0.5 bg-orange-100 text-orange-700 rounded-md text-xs font-black">{t('broadcasts.status.stopped', { processed: b.processed, total: b.total })}</span>}
                  {!isStopped && b.status === 'running' && <span className="px-2 py-0.5 bg-blue-100 text-blue-700 rounded-md text-xs font-black animate-pulse">{t('broadcasts.status.running')}</span>}
                  {!isStopped && b.status === 'queued' && b.processed === 0 && <span className="px-2 py-0.5 bg-slate-100 text-slate-500 rounded-md text-xs font-black">{t('broadcasts.status.queued')}</span>}
                  {!isStopped && b.status === 'failed' && <span className="px-2 py-0.5 bg-red-100 text-red-700 rounded-md text-xs font-black">{t('broadcasts.status.failed')}</span>}
                  {b.status === 'scheduled' && <span className="px-2 py-0.5 bg-blue-50 text-blue-500 rounded-md text-xs font-semibold">{t('broadcasts.status.waiting')}</span>}
                </span>
              </div>
            );
          })}
      ...
      {selectedBroadcast && (
        <div className="mt-6 bg-white border border-slate-100 rounded-3xl shadow-sm overflow-hidden">
          <div className="flex items-center justify-between gap-4 p-6 border-b border-slate-100">
            <div>
              <h2 className="text-xl font-black text-slate-900">{t('broadcasts.details.title')}</h2>
              <p className="text-xs text-slate-400 mt-1">
                {selectedBroadcast.group_name || t('broadcasts.details.custom')} ·{' '}
                {selectedBroadcast.createdAt
                  ? new Date(selectedBroadcast.createdAt).toLocaleString(getFormatLocale(i18n.resolvedLanguage), { dateStyle: 'short', timeStyle: 'short' })
                  : ''}
              </p>
            </div>
            <button
            ...
            >
              {t('broadcasts.details.close')}
            </button>
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 p-6">
            <div className="p-4 bg-slate-50 rounded-2xl text-center">
              <p className="text-xs font-bold text-slate-400 mb-1">{t('broadcasts.columns.total')}</p>
              <p className="text-3xl font-black text-slate-900">{selectedBroadcast.total}</p>
            </div>
            <div className="p-4 bg-green-50 rounded-2xl text-center">
              <p className="text-xs font-bold text-green-500 mb-1">{t('broadcasts.columns.sent')}</p>
            ...
            <div className="p-4 bg-red-50 rounded-2xl text-center">
              <p className="text-xs font-bold text-red-500 mb-1">{t('broadcasts.columns.failed')}</p>
            ...
            <div className="p-4 bg-amber-50 rounded-2xl text-center">
              <p className="text-xs font-bold text-amber-500 mb-1">{t('broadcasts.columns.skipped')}</p>
          ...
          <div className="p-6 border-t border-slate-100 text-sm text-slate-600">
            <p className="font-bold mb-2">{t('broadcasts.details.messageContent')}</p>
              ...
              <div className="p-4 bg-purple-50 rounded-2xl border border-purple-100">
                ...
                {selectedBroadcast.template_data?.params?.body?.length > 0 && (
                  <div className="text-xs text-slate-600">
                    <p className="font-bold mb-1">{t('broadcasts.details.parameters')}</p>
                ...
                {selectedBroadcast.template_data?.params?.header?.url && (
                  <p className="mt-3 text-xs text-slate-600 break-all">
                    <span className="font-bold">{t('broadcasts.details.media')}</span> {selectedBroadcast.template_data.params.header.url}
                  </p>
                )}
              </div>
            ) : (
          ...
          {selectedBroadcast.recipients?.length > 0 && (
            <div className="p-6 border-t border-slate-100">
              <p className="text-xs font-black text-slate-500 mb-3">{t('broadcasts.details.recipients', { count: selectedBroadcast.recipients.length })}</p>
                ...
                {selectedBroadcast.recipients.map((r: any, i: number) => (
                  <div key={i} className="flex items-center justify-between gap-3 px-4 py-3 bg-slate-50 rounded-2xl text-sm">
                    ...
                    <div className="text-xs font-semibold text-slate-500">
                      {r.reason === 'blocklist' && t('broadcasts.recipientReasons.blocklist')}
                      {r.reason === 'invalid_phone' && t('broadcasts.recipientReasons.invalidPhone')}
                      {r.error && t('broadcasts.recipientReasons.error', { error: r.error })}
                    </div>
                  </div>
                ))}
              </div>
